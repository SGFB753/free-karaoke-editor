// Editing during playback must aim at the NEXT line.
const { JSDOM } = await import('jsdom');
import fs from 'fs';
const dom = new JSDOM(fs.readFileSync(process.env.KARAOKE_PAGE_MIX, 'utf8'), {
  runScripts:'dangerously', pretendToBeVisual:true, url:'https://local.test/',
  beforeParse(w){
    w.__inst=[]; w.__errs=[];
    class FA{ constructor(){this.paused=true;this.volume=1;this.duration=26;this.seeking=false;
      this.playbackRate=1;this._t=0;this._h={};w.__inst.push(this);
      setTimeout(()=>this._fire('loadedmetadata'),0);}
      get currentTime(){return this._t;} set currentTime(v){this._t=v;this._fire('seeked');}
      addEventListener(n,f){(this._h[n]=this._h[n]||[]).push(f);} removeEventListener(){}
      _fire(n){(this._h[n]||[]).slice().forEach(f=>f());}
      play(){this.paused=false;return Promise.resolve();} pause(){this.paused=true;}}
    w.Audio=FA; w.onerror=m=>w.__errs.push(String(m));
    w.Element.prototype.getBoundingClientRect=function(){
      return {left:0,top:0,width:500,height:20,right:500,bottom:20,x:0,y:0}; };
    w.Element.prototype.setPointerCapture=function(){};
  }});
const w=dom.window,doc=w.document,$=id=>doc.getElementById(id),sleep=ms=>new Promise(r=>setTimeout(r,ms));
let saved=null;
w.URL.createObjectURL=()=>'blob:x'; w.URL.revokeObjectURL=()=>{};
w.Blob=class{constructor(p){saved=String(p[0]);}};
w.HTMLAnchorElement.prototype.click=function(){};
await sleep(200);
let fail=0; const ok=(n,c,e='')=>{console.log((c?'  ✓ ':'  ✗ ')+n+(e?' — '+e:'')); if(!c)fail++;};
const m=w.__inst[0];
const starts=()=>{ $('btnSaveJson').click(); return JSON.parse(saved).lines.map(l=>+l.start.toFixed(3)); };
const base=starts();
console.log('  the original starts:', base.join(', '));

console.log('\n--- “line starts here” pressed DURING AN INTERLUDE ---');
// after line 4 (ends around 13.6) comes a pause until line 5 (16.14).
// The line being edited is THE one shown highlighted — that is, the 4th.
m.currentTime=14.8; await sleep(80);
$('btnHere').click(); await sleep(60);
const after=starts();
ok('the highlighted fourth line was moved', Math.abs(after[3]-14.8)<0.05,
   `${base[3]} → ${after[3]}`);
ok('the fifth was left alone', after[4]===base[4], `${base[4]} → ${after[4]}`);
ok('its words begin at the new line start', (()=>{
   const l=JSON.parse(saved).lines[3]; return Math.abs(l.words[0].t-l.start)<0.02; })());
ok('the word lengths are kept (the fill will run smoothly)', (()=>{
   const l=JSON.parse(saved).lines[3]; return l.words.every(x=>x.d>0); })());

console.log('\n--- inside a sounding line the target does not change ---');
const b2=starts();
m.currentTime=9.0; await sleep(80);     // inside line 3 (8.00–10.6)
doc.dispatchEvent(new w.KeyboardEvent('keydown',{key:']',bubbles:true})); await sleep(60);
const a2=starts();
ok('the sounding line 3 is the one that moved', Math.abs(a2[2]-b2[2]-0.05)<1e-6,
   `${b2[2]} → ${a2[2]}`);
ok('the neighbours were not touched', a2[3]===b2[3]);

console.log('\n--- during the intro, before the first line ---');
const b3=starts();
m.currentTime=0.5; await sleep(80);     // the first line starts at 2.0
$('btnHere').click(); await sleep(60);
const a3=starts();
ok('the first line is edited, not “nothing”', Math.abs(a3[0]-0.5)<0.05,
   `${b3[0]} → ${a3[0]}`);

console.log('\n--- the “and all after it” box ---');
$('btnReset').click(); await sleep(60);
const b4=starts();
m.currentTime=14.8; await sleep(80);
$('chkRest').checked = true; $('btnHere').click(); await sleep(60); $('chkRest').checked=false;
const a4=starts();
ok('the lines before the highlighted one are untouched',
   [0,1,2].every(i=>Math.abs(a4[i]-b4[i])<1e-6));
const d4 = a4[3]-b4[3];
ok('the highlighted one and all after it moved by the same amount',
   [4,5].every(i=>Math.abs((a4[i]-b4[i])-d4)<1e-6), `shift ${d4.toFixed(2)}s`);
ok('no JS errors', w.__errs.length===0, w.__errs.join(';'));
console.log(fail?`\nFAILED: ${fail}`:'\nAll checks passed');
process.exit(fail?1:0);
